...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct 20 13:34:55 2022
...
def calculate():
    df_f1 = pd.read_csv("Stats/RIF_F1.csv")
    df_ari = pd.read_csv("Stats/RIF_ARI.csv")
    ...
    df_all = pd.merge(df_f1, df_ari,  how='left', left_on=["Filename","ntrees","standardize_data","sample_size","ncols_per_tree", "Parameter_Iteration"], right_on = ["Filename","ntrees","standardize_data","sample_size","ncols_per_tree", "Parameter_Iteration"])
    
    df_all.to_csv("Stats/RIF_Merged.csv", index=False)
    
    median_df = df_all.groupby(["ntrees","standardize_data","sample_size","ncols_per_tree"])[["F1_Median", "F1_Range", "ARI_Median"]].mean()
    median_df = median_df.reset_index()
    
    median_df.to_csv("Stats/RIF_Grouped_Median.csv", index=False)



def plot_ari_f1():
    median_df = pd.read_csv("Stats/RIF_Grouped_Median.csv")    
    df_all = pd.read_csv("Stats/RIF_Merged.csv")
    '''
    Plot Group Summary
    '''
    ...
    plt.savefig("Fig/RIF_F1_Range.pdf", bbox_inches="tight", pad_inches=0)
    plt.show()
    ...
    plt.ylabel("Performance (F1 Score)")
    plt.savefig("Fig/RIF_F1_ARI.pdf", bbox_inches="tight", pad_inches=0)
    plt.show()
    ...
    plt.xlabel("Determinism (ARI)")
    plt.ylabel("Performance (F1 Score)")
    plt.savefig("Fig/RIF_D_F1_ARI.pdf", bbox_inches="tight", pad_inches=0)
    plt.show()
...
def top5():
    ...
    df.to_csv("Stats/RIF_DefaultVSCS1.csv", index = False)
